      ...
      (async () => {
         ...
         const render = () => {
          petCard.innerHTML = `
            ...
            <button class="btn btn-success btn-outline" onclick="adoptPet(this)">Adopt</button>
            <button class="btn btn-success" onclick="showPetDetails('${pet.petId}')">Details</button>
            </div>
            </div>
          `;
          // Attach adoption process to the "Adopt" button
          const adoptBtn = petCard.querySelector('button.btn-outline[onclick^="adoptPet"]');
          if (adoptBtn) {
...
// Function to handle undefined or null text
...
// Initialize the functions to fetch and display data
cards_function();
pets_func();